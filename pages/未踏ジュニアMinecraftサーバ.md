
主に未踏関係者を中心とするJava版[[マインクラフト]]の限定公開サーバ
<img src='https://scrapbox.io/api/pages/nishio/nishio/icon' alt='nishio.icon' height="19.5"/>任意画像が貼れて外部サーバのWebhookを叩けて外部サーバからコマンドを実行できる、めちゃくちゃ自由度の高いメタバースになってきています！
参加したい人は西尾までご連絡ください。

[YouTube Playlist](https://www.youtube.com/playlist?list=PLbffFdG5769m3C8eTalSN1poyJ_wUSvb7)


歴史
- 2021-09-12にエイヤと作った
    - 未踏ジュニアのゲームチャットでマイクラサーバがあるといいなという話が盛り上がって
    - <img src='https://scrapbox.io/api/pages/nishio/nishio/icon' alt='nishio.icon' height="19.5"/>はJava版は未経験だったがせっかくサーバを管理するならプラグインで拡張できる方が面白いと考えてJava版にした
        - プラグインとMODの区別がついておらずバニラのサーバを立ててしまってからPaperMCのことを教えてもらって変更した
    - [[Transport-Pipes]]
- 2021-11-11 [[CybozuHackathon2021]]
    - このために作った
        - 10分間隔差分バックアップ
        - [[WheatSeedsSystem]]
            - バニラクライアントで既存のモデルを壊さずにカスタムモデルを追加できる仕組み
            - ![image](https://gyazo.com/4e1f5b8a682b983c7afc9ffbb4af7488/thumb/1000)

- 12月頭　1.18対応
    - 未踏ジュニアだけでなく他の未踏関係者にもオープン

- [[マイクラでエンジニアの知的生産術]]
    - ![image](https://gyazo.com/10b3ed940f93c2bbacbf03f3dbdf27fe/thumb/1000)
