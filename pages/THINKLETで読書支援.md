...
- 3: 深く知りたいキーワードを音声などで入力して、誌面の情報から聞きたいキーワードの正しい表記や文脈を把握して回答生成する
    - これもできる(動画にはないけど「エリートの過剰生産って何？」と質問して回答を得られた)
- 4: 現在の誌面だけではなく、過去の誌面もコンテキストに入れる
    - これをやるためには画像を全部載せるのは現実的ではないので質問が発生する前にOCRや要約などを走らせておく必要がある
- 5: 現在の書籍だけではなく、過去の書籍もコンテキストに入れる

レベル4を実現するためには、音声トリガーした時に初めて撮影・OCRをするのではなくボタンなどで読書モードに入り、その後継続的に撮影とOCRを繰り返して蓄積していくことが必要
- ここをSolでやったら1回あたり15円とか掛かるので、安いOCRをかけて状況を把握し、新しいページが読みやすく撮影されてる時だけSolでやるのがいい(安いOCRで十分ならそもそもSolなしでもいい)
